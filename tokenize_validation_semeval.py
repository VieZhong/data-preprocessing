...
tokenized_dir = "/data/semeval/validation_json_token/tokenized"
finished_files_dir = "/data/semeval/validation_json_token/"


def write_file_to_tmp(text_path, keyword, file_name):
  text_lines = read_text_file(text_path)
  
  title_index = 0
  if "ABSTRACT" in text_lines:
    abstract_index = text_lines.index("ABSTRACT") + 1
  elif  "Abstract" in text_lines:
    abstract_index = text_lines.index("Abstract") + 1
  elif "abstract" in text_lines:
    abstract_index = text_lines.index("abstract") + 1
  else:
...
def tokenize_stories(file_dir, tokenized_dir):
  ...
  json_files = os.listdir(os.path.join(file_dir, 'test'))
  names = []

  key_file = os.path.join(file_dir, 'test_answer', 'test.author.stem.final')
  key_lines = read_text_file(key_file)
  key_dict = dict()
  for line in key_lines:
    index = line.index(':')
    keys = [key.strip() for key in line[index + 1:].split(',')]
    key_dict[line[:index].strip()] = ';'.join(keys)

  for j_file in json_files:
    name = j_file.split('.')[0]
    if name in key_dict:
      names.append(j_file)
      write_file_to_tmp(os.path.join(file_dir, 'test', j_file), key_dict[name], j_file)
